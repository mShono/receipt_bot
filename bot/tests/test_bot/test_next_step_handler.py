...
from ...messages import RECOGNIZED_PRODUCTS_MISSING_IN_DATABASE, UPLOAD_EXPENSE, UNSUCCESSFUL_UPLOAD_EXPENSE, SUCCESSFUL_CATEGORY_CORRECT, SUGGESTION_TO_ENTER_CATEGORY_FOR_PRODUCT
...
def run_press_nothing_saves_expence_test(
    ...
    assert message_text == UPLOAD_EXPENSE
...
def test_press_existing_category_post_the_expense(monkeypatch, mock_post_data_info_positive, fake_callback_query_factory, fake_context, mock_send_message):
...
def test_press_existing_category_sends_absent_buttons(monkeypatch, mock_post_data_info_positive, fake_callback_query_factory, fake_context, mock_send_message):
    context = fake_context
